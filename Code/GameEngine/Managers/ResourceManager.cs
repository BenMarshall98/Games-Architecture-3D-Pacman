...
namespace Game_Engine.Managers
{
    public static class ResourceManager
    {
        ...
        static Dictionary<string, CubeMapTexture> cubemapDictionary = new Dictionary<string, CubeMapTexture>();
        ...
        public static Shader LoadShader(string vertexShader, string fragmentShader)
        {
            Shader shader;
            string concat = vertexShader + fragmentShader;
            shaderDictionary.TryGetValue(concat, out shader);
            if (shader == null)
            {
                shader = new Shader();
                shader.LoadShader(vertexShader, fragmentShader);
                shaderDictionary.Add(concat, shader);
            }
        ...
        private static byte[] LoadWave(FileStream stream, out int channels, out int bits, out int rate)
        {
            if (stream == null)
